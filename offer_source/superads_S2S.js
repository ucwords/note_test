...
function getPage(page) {
    return new Promise(async (ac, rj) => {
        let offers = [];
    try {
        let data = await rp('http://offer.superads.cn/v1/jsons?token=0284e3f26a3c1b2ae2c1c0d4f3c44f38&u=Leanmobi1&page_size=100&page=1');
        //console.log(data);
        ...
        let offer_ids = await rp('http://portal.leanmobi.com/?s=admin/auto_pull/getOfferIdsByAdvName&adv=superads_S2S');
        ...
        for (let i in data.offers) {
            ...
            if (tmpOffer.package_name.indexOf(".") > 0) {
                ...
                tmpOffer.platform = 'ios';  // {idfa}  {gaid}
                tmpOffer.package_name = tmpOffer.package_name.replace('id', '');
            }
            tmpOffer.adv_url = adv_offer.link+'&click_id={clickid}&ch_subid={network_id}_{sub_id}&idfa={idfa}&gaid={gaid}';

            tmpOffer.kpi = adv_offer.notes;
