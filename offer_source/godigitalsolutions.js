...
exports.getOffer = function(){
    return new Promise(async function(ac,rj){  //resolve, reject  async表示函数里有异步操作
        try{
            console.log('godigitalsolutions');
            let maxpage = 1;
            let offers=[];
            let p_offers=[];
...
function getPage(page) {
    return new Promise(async (ac, rj) => {
        let offers = [];
    try {
        let data = await rp('http://partner.goapi.in/api.php?affid=157&api-key=ZGC28qITP2ZIJ2s&status=active');
        //console.log(data);
        ...
        for (let i in data.response) {
            ...
            tmpOffer.daily_cap = getCap(adv_offer.dailycap);
            ...
            tmpOffer.adv_url = adv_offer.landing_url.replace('{click_id}','{clickid}').replace('{affiliate_id}', '{network_id}_{sub_id}').replace('{ifa_id}', '{idfa}').replace('{google_id}', '{gaid}');
