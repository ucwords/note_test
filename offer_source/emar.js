...
function getPage(page) {
 return new Promise(async (ac, rj) => {
    let offers = [];
    try {
        let data = await rp('http://cpisvr.emarbox.com/offer?token=030251078af8789256a7df6ea06ddc16&size=1000');
        //console.log(data);
        ...
        for (let i in data.data.offers) {
            ...
            if (adv_offer.clickUrl == null) {
                continue;
            }
            tmpOffer.payout = adv_offer.payout+'';

            let geo = adv_offer.geo2b;
            ...
            tmpOffer.adv_url = adv_offer.clickUrl + '&extraInfo1={clickid}&sub={network_id}_{sub_id}&gaid={gaid}&idfa={idfa}';
